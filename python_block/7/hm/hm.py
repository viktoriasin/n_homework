...
user_item_query_config = {
    "MIN_USERS_RATING_FOR_ITEM": 3
}

sql_str = ("""
SELECT *  INTO movies_top FROM (
    SELECT MOVIEID, COUNT(RATING)  ratings_num, AVG(RATING) ratings_avg 
    FROM RATINGS
    GROUP BY MOVIEID
...
# As much as possible, regular SQLAlchemy schema and ORM constructs are used directly, so that configuration between “classical” ORM usage and declarative remain highly similar.
